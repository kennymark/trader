...
import { isDeepSeekEnabled } from "./deepseek.js";
import {
  daysUntil,
  getAnalystSnapshot,
  getCalendar,
...
import { buildAiStockAnalysis, enrichHuntRationales } from "./intelligence/aiAnalyst.js";
import { buildMarketExpectations } from "./intelligence/expectations.js";
import {
  detectHappening,
  returnOverBars,
  volumeSpikeRatio,
} from "./intelligence/happening.js";
import { findHistoricalPatterns } from "./intelligence/patterns.js";
import { buildPortfolioHealth } from "./intelligence/portfolio.js";
import {
  loadPriorScores,
  persistOpportunitySnapshots,
  recordPredictionsFromHunt,
} from "./intelligence/predictions.js";
import {
  defaultAssumptionsFromFundamentals,
  simulateScenarios,
} from "./intelligence/scenarios.js";
import { computeOpportunityScores } from "./intelligence/scoring.js";
...
export async function buildIntelligence(
  ...
  if (opts?.userId) {
    try {
      priorScores = await loadPriorScores(opts.userId);
    } catch (err) {
  ...
  if (opts?.userId) {
    try {
      const { listPortfolioHoldings } = await import("./freetradeImport.js");
      const holdings = await listPortfolioHoldings(opts.userId);
  ...
  if (opts?.userId && opts.persist !== false) {
    try {
      await persistOpportunitySnapshots(opts.userId, enriched);
      await recordPredictionsFromHunt(opts.userId, enriched);
    } catch (err) {
